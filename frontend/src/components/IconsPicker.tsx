...
export default function IconsPicker({
  ...
  const totalPages = Math.ceil(iconsList.length / itemsPerPage);
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentIcons = iconsList.slice(startIndex, endIndex);
  ...
  return (
    <motion.div
      ...
      className={cn(
        "bg-neutral-50 rounded-2xl shadow-sm ring-1 ring-black/5 p-2 dark:bg-neutral-900 dark:ring-white/5",
      )}
    >
      <div
          ...
          {currentIcons.map((icon, idx) => {
            ...
            return (
              <div
                key={idx}
                className={cn(
                  "w-10 h-10 rounded-lg cursor-pointer transition-all duration-300",
